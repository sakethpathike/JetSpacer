package com.sakethh.jetspacer.screens.space.rovers.opportunity.cameras.pancam
...
@Composable
fun PANCAMOpportunityCameraScreen(){
    val opportunityVM: OpportunityCamerasVM = viewModel()
    val roversScreenVM: RoversScreenVM = viewModel()
    val coroutineScope = rememberCoroutineScope()
    val solImagesData = opportunityVM.pancamDataFromAPI.value
    LaunchedEffect(key1 = true) {
        opportunityVM.retrieveOpportunityCameraData(
            cameraName = OpportunityCamerasVM.OpportunityCameras.PANCAM,
            sol = PANCAMOpportunityCameraScreen.solValue.value.toInt(),
            page = 0
        )
    }
    Scaffold(floatingActionButtonPosition = FabPosition.Center, floatingActionButton = {
        if (solImagesData.isNotEmpty() && opportunityVM._pancamDataFromAPI.value.isEmpty() && opportunityVM.isPancamDataLoaded.value && roversScreenVM.atLastIndexInLazyVerticalGrid.value) {
            Snackbar(
        ...
        Column(
            ...
            SolTextField(solValue = PANCAMOpportunityCameraScreen.solValue, onContinueClick = {
                PANCAMOpportunityCameraScreen.currentPage = 0
            ...
            if (!opportunityVM.isPancamDataLoaded.value) {
                StatusScreen(
                    title = "Wait a moment!",
                    description = "fetching the images from this camera that were captured on sol ${PANCAMOpportunityCameraScreen.solValue.value}",
                ...
                StatusScreen(
                    title = "4ooooFour",
                    description = "No images were captured by this camera on sol ${PANCAMOpportunityCameraScreen.solValue.value}. Change the sol value; it may give results.",
                    status = Status.FOURO4InMarsScreen
                )
            } else {
                ModifiedLazyVerticalGrid(
                    listData = solImagesData,
                    loadMoreButtonBooleanExpression = opportunityVM._pancamDataFromAPI.value.isNotEmpty() && opportunityVM.isPancamDataLoaded.value
                ) {
                    coroutineScope.launch {
                        opportunityVM.retrieveOpportunityCameraData(
                            cameraName = OpportunityCamerasVM.OpportunityCameras.PANCAM,
                            sol = PANCAMOpportunityCameraScreen.solValue.value.toInt(),
                            page = PANCAMOpportunityCameraScreen.currentPage++
                        )
                    }
                }
            }
...
object PANCAMOpportunityCameraScreen {
