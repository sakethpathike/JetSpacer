...
class SpiritCamerasImplementation : SpiritCamerasService {
    override suspend fun getRandomCamerasData(sol: Int, page: Int): RandomCameraDTO {
       return HTTPClient.ktorClient.get("https://api.nasa.gov/mars-photos/api/v1/rovers/spirit/photos?sol=$sol&page=$page&api_key=${Constants.NASA_APIKEY}")
            .body()
    }

    override suspend fun getFHAZData(sol: Int, page: Int): List<Photo> {
        return specificRoverHTTPRequest(roverName = "spirit", cameraName = "fhaz",sol=sol, page = page).component1()
    }

    override suspend fun getRHAZData(sol: Int, page: Int): List<Photo> {
        return specificRoverHTTPRequest(roverName = "spirit", cameraName = "rhaz",sol=sol, page = page).component1()
    }


    override suspend fun getNAVCAMData(sol: Int, page: Int): List<Photo> {
        return specificRoverHTTPRequest(roverName = "spirit", cameraName = "navcam",sol=sol, page = page).component1()
    }

    override suspend fun getPANCAMData(sol: Int, page: Int): List<Photo> {
        return specificRoverHTTPRequest(roverName = "spirit", cameraName = "pancam",sol=sol, page = page).component1()
    }

    override suspend fun getMINITESData(sol: Int, page: Int): List<Photo> {
        return specificRoverHTTPRequest(roverName = "spirit", cameraName = "minites",sol=sol, page = page).component1()
    }
}